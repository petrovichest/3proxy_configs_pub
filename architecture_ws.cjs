...
function check(row) {
  return new Promise(resolve => {
    ...
    const socket = new WebSocket(`wss://[${fixture.ipv6}]:18443/ws`, {
      ...
      // Use the fixture certificate's name for SNI even though CONNECT uses its IP.
      servername: 'proxy-architecture-fixture',
      headers: {'X-Lab-Token': fixture.token}, handshakeTimeout: 15000,
    });
    let count = 0, finished = false;
    const timer = setTimeout(() => { errors.timeout = (errors.timeout || 0) + 1; finish(false); }, 20000);
    function finish(ok) {
      if (finished) return;
      finished = true;
      clearTimeout(timer);
    ...
    socket.on('message', raw => {
      try {
        ...
        if (normalize(data.exit) !== normalize(row.ipv6)) {
          errors.wrong_exit = (errors.wrong_exit || 0) + 1;
          return finish(false);
        }
...
(async () => {
  ...
  console.log(JSON.stringify({node_ws: {passed, failed: results.length - passed, errors}, logical_proxies: pool.length}));
