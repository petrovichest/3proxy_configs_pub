...
1.  Запустите скрипт:
    ...
    ```
2.  Следуйте инструкциям в консоли. Скрипт запросит SSH-хост, общее количество прокси, подсеть IPv6 и сетевой интерфейс. Префикс пачек задаётся через `--project-prefix` (по умолчанию `capacity`); пользователь — через `--user` (по умолчанию `root`). Для парольного доступа используется `--password`.

**Что происходит:**
Удалённый установщик рассчитан на Ubuntu/Debian и root-доступ. Скрипт подключится по SSH, при необходимости установит Git, клонирует или обновит репозиторий, установит зависимости и 3proxy, добавит недостающие конфигурации, запустит службы `systemd` и проверит выходные IPv6. С `--skip-install` необходимые пакеты должны быть установлены заранее.

**Результат:**
На локальной машине появится `downloaded_configs/<host>/extracted_proxy` с общим списком. В подкаталогах пачек сохраняются отдельные списки, таблицы назначенных IPv6 и `proxy_check_results.txt` с результатами проверки.
...
2.  **Результаты генерации** (в директории `generated_proxy_configs/<имя_проекта>/` - *использование разных имен позволяет создавать и управлять несколькими независимыми пачками прокси на одном сервере*):
    ...
    *   `setup_network_ipv6.sh`: Скрипт для привязки адресов проекта к готовой IPv6 сети.
...
*   **Привязать/отвязать конкретный IPv6-адрес**:
    ...
    Адрес должен принадлежать указанному проекту; основной адрес сервера этим способом не удаляется.
...
Для отдельного сравнения аллокатора доступен `--malloc-arenas 2`; значение `0`
возвращает системное поведение. Этот параметр требует `--start` и перезапускает
только службы, у которых меняется настройка. По умолчанию аллокатор не меняется.
